...
for i in Temp:
    EM=PU.get_Ep(i,freq)
    plot_label='T = '+str(i)
    plt.plot(freq,EM,label=plot_label)
    plt.title('Storage Modulus [MPa]')    
...
plt.subplot(1,2,2)    
for i in Temp:
    EM=PU.get_Epp(i,freq)
    plot_label='T = '+str(i)
    plt.semilogx(freq,EM,label=plot_label)
	    
    plt.title('Loss Modulus [MPa]')    
    plt.xlabel('Frequency [Hz]')
    plt.legend(loc = 'best')
    print()
plt.grid()   